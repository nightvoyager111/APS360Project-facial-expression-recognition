...
class EmotionAlexNet(nn.Module):
    def __init__(self, num_classes=7, use_residual=False):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(1, 64, kernel_size=5, stride=1, padding=2),  # smaller kernel, stride
            nn.BatchNorm2d(64),
            ...
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
            # Remove or comment out the last MaxPool2d
...
test_data = datasets.ImageFolder('fer2013plus/fer2013/test', transform=transform)

# RAF-DB example
# train_data = datasets.ImageFolder('RAF-DB/train', transform=transform)
# test_data = datasets.ImageFolder('RAF-DB/test', transform=transform)

train_loader = DataLoader(train_data, batch_size=50, shuffle=True, num_workers=2)
val_loader = DataLoader(test_data, batch_size=50, shuffle=False, num_workers=2)
