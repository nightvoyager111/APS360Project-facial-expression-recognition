...
def visualize_hog_only():
    # Use FERPlus test set, 'happiness' class, second image
    classes = get_classes(FERPLUS_TRAIN_DIR, FERPLUS_TEST_DIR)
    happiness_class = 'happiness'
    class_dir = os.path.join(FERPLUS_TEST_DIR, happiness_class)
    img_files = [f for f in os.listdir(class_dir) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
    if not img_files or len(img_files) < 2:
        print("Not enough images found in happiness test class directory.")
        return
    img_path = os.path.join(class_dir, img_files[1])
    img = imread(img_path, as_gray=True)
    img_resized = resize(img, IMAGE_SIZE)
    # HOG features and visualization
    _, hog_image = hog(img_resized, visualize=True, **HOG_PARAMS)
    # Plot
...
def visualize_svm_scores_for_sample():
    # Use FERPlus test set, 'happiness' class, second image
    classes = get_classes(FERPLUS_TRAIN_DIR, FERPLUS_TEST_DIR)
    class_to_idx = {cls: idx for idx, cls in enumerate(classes)}
    happiness_class = 'happiness'
    ...
    img_resized = resize(img, IMAGE_SIZE)
    # HOG features and visualization
    hog_feat, hog_image = hog(img_resized, visualize=True, **HOG_PARAMS)
    # Load or train SVM
    model_path = 'FERPlus_hog_svm.joblib'
    if os.path.exists(model_path):
        clf, scaler, target_names = load(model_path)
    else:
        # Fallback: train on the fly (quick, on small subset)
        X_train_img, y_train = load_image_folder(FERPLUS_TRAIN_DIR, classes, class_to_idx)
        X_train = extract_hog_features(X_train_img)
        scaler = StandardScaler()
        X_train = scaler.fit_transform(X_train)
        clf = LinearSVC(dual=False, max_iter=2000)
    ...
    axs[1].set_title('HOG Visualization')
    axs[1].axis('off')
    # Bar chart for SVM scores
    bar_colors = ['tab:blue'] * len(scores)
    bar_colors[pred] = 'tab:red'
    axs[2].bar(range(len(scores)), scores, color=bar_colors)
    axs[2].set_xticks(range(len(scores)))
    axs[2].set_xticklabels(target_names, rotation=45, ha='right')
    axs[2].set_title(f'SVM Scores\nPredicted: {pred_label}')
...
def main():
    ...
    visualize_svm_scores_for_sample()
    plt.show()
